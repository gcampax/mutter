  ...
  for (i = 0; i < output_kms->connector->count_props; i++)
    {
      ...
      if ((prop->flags & DRM_MODE_PROP_BLOB) &&
          ...
          prop->count_blobs >= 1)
        {
          output_kms->edid_blob_id = prop->blob_ids[0];
          drmModeFreeProperty(prop);
          break;
        }
                  ...
                  MetaOutputKms         *output_kms)
{
  ...
  if (edid_blob && edid_blob->length > 0 && edid_blob->length % 128 == 0)
  ...
  for (i = 0; i < manager_kms->n_connectors; i++)
      ...
      if (connector->connection == DRM_MODE_CONNECTED)
	{
	  ...
	  meta_output->is_presentation = FALSE;
          ...
          edid = read_output_edid (manager_kms, output_kms);
          if (edid)
            {
                                    ...
                                    MetaOutput         *output)
{
  ...
  return read_output_edid (manager_kms, output->driver_private);
}
                                              ...
                                              unsigned int         n_outputs)
{
  ...
  error = NULL;
  ok = cogl_kms_display_set_layout (cogl_display, screen_width, screen_height, cogl_crtcs, &error);
